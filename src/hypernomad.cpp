...
// For testing getNeighboors
        ...
        if ( stopType == X0_FAIL )
            cerr << endl << "The starting point cannot be evaluated. Please verify that all pytorch scripts are available. The default setting for bbExe (" << p.get_bb_exe().front() << " ) maybe incorrect. Read the out.txt file for more details." << endl << endl;
